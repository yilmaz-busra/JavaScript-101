// prompt ile aldıgım not bilgisine gore #info ya yazdır
...
let textInfo;
let gradeInfo=document.querySelector('#info')


if(examGrade <= 100 && examGrade >= 0){
    textInfo = SMILE
    gradeInfo.classList.add("text-primary")
    if(examGrade >= 90){
        textInfo += "AA"
        ...
        textInfo = ${SAD} FF
        gradeInfo.classList.remove("text-primary")
        gradeInfo.classList.add("text-danger")
    }
}else {
    textInfo = "Geçersiz Sınav Sonucu"
}
gradeInfo.innerHTML=`${textInfo} fvmdsk`
